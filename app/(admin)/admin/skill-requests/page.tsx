...
//     const [clearedDrafts, setClearedDrafts] = useState<
//         Record<string, SkillDraft>
//     >({});
...
//     if (failed) {
//         return (
//             <div className="flex h-28 items-center justify-center rounded-lg border border-red-200 bg-red-50 px-2 text-center text-xs text-red-500">
...
//     return (
//         <div className="flex h-28 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-slate-50 p-2">
//             <img
//                 src={src}
//                 alt={alt}
...
interface SavedSkillDrafts {
    drafts: Record<string, SkillDraft>;
    clearedDrafts: Record<string, SkillDraft>;
}
...
const SKILL_DRAFT_STORAGE_KEY =
    "narp_admin_skill_request_drafts";
...
export default function SkillRequestsPage() {
    ...
    /*
     * Drafts currently saved for each SkillRequest.
     *
     * Key = SkillRequests document ID
     */
    const [skillDrafts, setSkillDrafts] = useState<
        Record<string, SkillDraft>
    >({});
    ...
    /*
     * Stores the previous version when the admin clicks Clear.
     *
     * This allows Undo Clear.
     */
    const [clearedDrafts, setClearedDrafts] = useState<
        Record<string, SkillDraft>
    >({});
    ...
    /*
     * Prevent the initial localStorage load from being
     * immediately overwritten by the save effect.
     */
                ...
                const data: SkillRequest[] =
                    snapshot.docs.map((item) => {
                        ...
                        return {
                            ...
                            votes: Number(raw.votes ?? 0),
                            requestedUsers:
                                Array.isArray(raw.requestedUsers)
                                    ? raw.requestedUsers
                                    : [],
                            status: raw.status ?? "",
                            createdAt: raw.createdAt,
        ...
        try {
            const saved =
                window.localStorage.getItem(
                    SKILL_DRAFT_STORAGE_KEY
                );

            if (saved) {
                const parsed: SavedSkillDrafts =
                    JSON.parse(saved);
                ...
                if (
                    ...
                    setClearedDrafts(
                        parsed.clearedDrafts ?? {}
                    );
                }
            }
        } catch (error) {
            console.error(
                "Failed to load skill drafts from localStorage:",
                error
            );
        } finally {
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.error(
                "Failed to save skill drafts to localStorage:",
                error
            );
        }
    }, [
    ...
    const rejectRequest = async (
        ...
        try {
            ...
            /*
             * The request is no longer pending, so its
             * temporary draft is no longer needed.
             */
            removeSkillDraft(
                request.id
            );
    ...
    const approveSkill = async (
        ...
        try {
            ...
            /*
             * Remove the temporary local draft because
             * the skill is now permanently saved.
             */
            removeSkillDraft(
                request.id
            );
...
function ApproveSkillModal({
    ...
    /*
     * Initialize from the saved draft if one exists.
     *
     * This is the key part that allows:
     *
     * Generate → Cancel → Reopen
     *
     * to restore all generated information.
     */
    const [form, setForm] =
        useState<ApproveForm>(() => ({
            title:
                draft?.title ??
                request.skillName,
    ...
    /*
     * Keep local modal state synchronized if the
     * parent provides a different saved draft.
     */
    useEffect(() => {
        if (!draft) {
            return;
    ...
    const updateField = (
        ...
        setForm(
            (current) => {
                ...
                /*
                 * Save every manual edit immediately.
                 *
                 * This means even if the admin types something
                 * manually and closes the modal, it remains.
                 */
                onDraftChange({
                    title:
                        updated.title,
    ...
    const handleGenerate = async () => {
        /*
         * If content already exists, this is now a
         * deliberate regeneration.
         *
         * Ask for confirmation so the admin doesn't
         * accidentally spend another AI generation.
         */
        if (
            hasGeneratedContent
        ) {
            const confirmed =
                window.confirm(
                    "This skill already has generated information.\n\nGenerate a new version and replace the current description and images?"
                );
        ...
        /*
         * Save the current version before generating.
         */
        onDraftChange(
            getCurrentDraft()
        );
        ...
        try {
             ...
             * IMPORTANT:
             *
             * Immediately save the generated data
             * into localStorage through the parent.
             */
            onDraftChange({
                title:
                    generatedForm.title,
    ...
    const handleClear = () => {
        /*
         * Save the current version so Undo Clear
         * can restore it.
         */
        onClear(
            getCurrentDraft()
         ...
         * Keep the gender.
         *
         * Clear is primarily intended to remove
         * generated/form content. The admin's selected
         * gender remains available.
         */
        onDraftChange({
            title:
                request.skillName,
    ...
    const handleUndoClear = () => {
        ...
        const restoredForm:
            ...
            description:
                clearedDraft.description,

            image:
                clearedDraft.image,

            sampleImage:
                clearedDraft.sampleImage,
        };
                        ...
                        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-blue-700">
                                        {hasGeneratedContent
                                            ? "AI content already exists. Generate again only if you want to replace it."
                                            : "Generate the description and images automatically."}
                                    </p>
                                </div>
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-2">
                            ...
                            {clearedDraft && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-white px-4 py-2.5 text-sm font-semibold text-blue-600 transition hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-50"
                                >
